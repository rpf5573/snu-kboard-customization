...
<tr class="<?php if($content->uid == kboard_uid()):?>kboard-list-selected<?php endif?>">
	...
	<?php if($board->use_category && $board->initCategory1()):?>
		...
		<?php if($board->initCategory1() && $content->category1):?>
			<a href="<?php echo $url->setPath(site_url().'/wp-admin/admin.php')->set('page', 'kboard_admin_view_'.$board->id)->set('kboard_id', $board->id)
			->set('category1', $content->category1)->set('pageid', '1')->set('target', '')->set('keyword', '')->set('mod', 'list')->toString()?>" title="<?php echo $content->category1?>"><div class="category1-wrap"><?php echo $content->category1?></div></a>
		<?php endif?>
		</td>
	<?php endif?>
	<td class="kboard-list-title">
		...
		<div class="kboard-mobile-contents">
			...
			<span class="contents-item kboard-category2">
				<a href="<?php echo $url->setPath(site_url().'/wp-admin/admin.php')
		 			->set('page', 'kboard_admin_view_'.$board->id)
		 			->set('kboard_id', $board->id)
					->set('category2', $content->category2)->set('pageid', '1')->set('mod', 'list')->toString()?>" title="<?php echo $content->category2?>"><?php echo $content->category2?>
				</a>
			</span>
	...
	<td class="kboard-list-category2">
		<?php if($board->initCategory2() && $content->category2):?>
			<a href="<?php echo $url->setPath(site_url().'/wp-admin/admin.php')->set('page', 'kboard_admin_view_'.$board->id)->set('kboard_id', $board->id)
			->set('category2', $content->category2)->set('pageid', '1')->set('mod', 'list')->toString()?>" title="<?php echo $content->category2?>"><?php echo $content->category2?></a>
		<?php endif?>
	</td>
	<?php endif?>
